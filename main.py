...
CONFIG = {
    ...
    "parked_threshold": 4,
    "tolerance": 1.5,
    "prediction_strength": 0.3,  # Увеличенный параметр экспоненциального снижения
    "path_thickness": 1,
    "center_dot_radius": 2,
    "object_lifetime": 2
}
...
def get_inverted_triangle_points(x1, y1, x2, y2):
    # Верхняя точка треугольника (средняя по x, нижняя по y)
...
def run_video_analysis(video_path):
    # Загрузка моделей
    model_traffic = YOLO('модель для анализа движения.pt')  # Модель для анализа движения
    model_crash = YOLO('модель для обнаружения аварий.pt')  # Модель для обнаружения аварий
    ...
    direction_colors = {
        'up': (0, 255, 0),       # Зеленый
        'down': (255, 0, 0),     # Синий
        'left': (0, 0, 255),     # Красный
        'right': (255, 255, 0),  # Желтый
        'diagonal': (255, 0, 255),  # Пурпурный
        'none': (200, 200, 200)  # Серый
    }
    ...
    while cap.isOpened():
        ...
        # Анализ движения
        processed_frame, tracked_objects, flow_history = frame_process(
            frame, model_traffic, tracked_objects, flow_history
        )

        # Рисование путей и направлений
        draw_paths_and_centers(processed_frame, tracked_objects, direction_colors)

        # Анализ аварий
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        crash_results = model_crash.predict(source=rgb_frame, conf=0.5)
        crash_count = 0
        ...
        for result in crash_results:
            ...
            for box in boxes:
                ...
                if confidence > 0.5 and int(box.cls[0]) == 0:
                    crash_count += 1
                    triangle_points = get_inverted_triangle_points(x1, y1, x2, y2)
                    draw_inverted_triangle(processed_frame, triangle_points, color=(0, 0, 255), thickness=3)

        # Отображение текста о количестве аварий
        pil_image = Image.fromarray(cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB))
        draw = ImageDraw.Draw(pil_image)
        font = ImageFont.truetype(CONFIG["font_path"], 20)

        text = f"Аварии: {crash_count}"
        text_bbox = draw.textbbox((15, 50), text, font=font)
        draw.rectangle([(15, 50), (text_bbox[2], text_bbox[3])], fill=(0, 0, 0, 128))
        draw.text((15, 50), text, fill=(255, 255, 255), font=font)

        processed_frame = cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)

        # Показать результат
        cv2.imshow('Traffic and Crash Analysis', processed_frame)

        if cv2.waitKey(1) & 0xFF == ord('q'):
            print("Остановка обработки и закрытие окна...")
            break
    cap.release()
...
if __name__ == "__main__":
    video_path = "видео с движением автомобилей.mp4"  # Замените на путь к вашему видео
